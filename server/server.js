...
app.post('/captureYourself', upload.single('image'), async (req, res) => {
  try {
    ...
    console.log("Calling Python script for embedding generation...");
    exec(`python generate_embedings.py "${resizedBase64Image}"`, async (error, stdout, stderr) => {
      if (error) {
      ...
      console.log("Embedding generated by Python:", stdout);
      let embedding;
      ...
      try {
        ...
        embedding = stdout.trim().split(',').map(val => parseFloat(val.trim())).filter(val => !isNaN(val));
        if (embedding.length === 0) {
          console.error("Embedding format invalid or empty.");
          return res.status(400).json({ message: 'Invalid or empty embedding' });
        }
      }
      console.log("Parsed embedding:", embedding);

      const user = await User.findOne({ email: 'embeddingtest@iitj.ac.in' });
      if (!user) {
        console.error("User with email 'embeddingtest@iitj.ac.in' not found in database");
        return res.status(404).json({ message: 'User not found' });
      }

      console.log("User's embedding from database:", user.embedding);
      const similarityScore = similarity(embedding, user.embedding);
      console.log("Calculated similarity:", similarityScore);

      const attendanceMarked = similarityScore >= 0.65;
      const statusMessage = attendanceMarked ? 'Attendance marked' : 'Attendance not marked';

      // Save the attendance record in MongoDB
